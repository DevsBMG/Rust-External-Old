...
geo::vec2_t moveTowards_2d(const geo::vec2_t& pointA, const geo::vec2_t& pointB, float distance) {
	geo::vec2_t direction(pointB.x - pointA.x, pointB.y - pointA.y);
	...
	geo::vec2_t newPosition(pointA.x + direction.x * distance, pointA.y + direction.y * distance);
